...
interface AppDataContextType {
  ...
  completeMaintenance: (id: number) => Promise<void>;
  uncompleteMaintenance: (id: number) => Promise<void>;
}
...
export function AppDataProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      setTreatments(
        data.map((t: any) => ({
          ...
          actualDate: t.actualDate ? new Date(t.actualDate) : undefined,
        })),
      );
    } catch (err) {
    ...
    try {
      ...
      setVehicles(
        data.map((v: any) => ({
          ...
          insuranceDate: v.insuranceDate ? new Date(v.insuranceDate) : undefined,
          roadLegalUntil: v.roadLegalUntil ? new Date(v.roadLegalUntil) : undefined,
        })),
      );
    } catch (err) {
  ...
  const fetchEquipment = useCallback(async () => {
    ...
    try {
      ...
      setEquipment(
        data.map((e: any) => ({
          ...e,
          verificationDate: new Date(e.verificationDate),
    ...
    try {
      ...
      setMaintenance(
        data.map((m: any) => ({
          ...
          hours: m.hours != null ? Number(m.hours) : undefined,
  ...
  const updateShipment = useCallback(
    async (id: number, data: any) => {
      const res = await fetch(`${getBaseUrl()}/shipments/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
  ...
  const updateTreatment = useCallback(
    async (id: number, data: any) => {
      const res = await fetch(`${getBaseUrl()}/treatments/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
  ...
  const uncompleteTreatment = useCallback(
    async (id: number) => {
  ...
  const updateVehicle = useCallback(
    async (id: number, data: any) => {
      const res = await fetch(`${getBaseUrl()}/vehicles/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
  ...
  const updateEquipment = useCallback(
    async (id: number, data: any) => {
      const res = await fetch(`${getBaseUrl()}/equipment/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
  ...
  const uncompleteMaintenance = useCallback(
    async (id: number) => {
      await updateMaintenance(id, {
        completed: false,
        actualDate: null,
